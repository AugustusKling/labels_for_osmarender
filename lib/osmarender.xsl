...
<xsl:stylesheet
  ...
  xmlns:osm="http://openstreetmap.org.sample/osm/0.6"
  ...
  <xsl:param name="grouped" select="/rules/@grouped"/>
  ...
  <xsl:template name="drawCircle">
    ...
    <xsl:message>Emitting circle.</xsl:message>
  ...
  <xsl:template name="getSvgAttributesFromOsmTags">
	...
	<!-- Builds z:command including OSM id, drawing instruction and layer -->
  ...
  <xsl:template match="*" mode="circle">
  	<xsl:message>Match all, mode circle to suppress.</xsl:message>
  </xsl:template>
  ...
  <xsl:template match="way" mode="circle">
    ...
    <xsl:message>Circle for way.</xsl:message>
  ...
  <xsl:template match="relation" mode="circle">
    ...
    <xsl:message>Circle for relation.</xsl:message>
  ...
  <xsl:template name="areaCenterWrapper">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
				<xsl:value-of select="$data/osm/node[@id=$element/nd/@ref]/@lon" />,<xsl:value-of select="$data/osm/node[@id=$element/nd/@ref]/@lat" />
...
<xsl:template name="processRules">
	...
	<!-- Apply rules as defined in style file -->
	...
	<xsl:for-each select="exslt:node-set($commands)/z:command">
		<xsl:call-template name="renderCommand">
			<xsl:with-param name="command" select="."/>
		</xsl:call-template>
	</xsl:for-each>
	<!--<xsl:for-each select="$elements">
		<xsl:variable name="osmId" select="@id"/>
		<xsl:variable name="renderingCommandsForElement" select="exslt:node-set($commands)/z:command[z:element[@id=$osmId]]"/>
		
		<xsl:variable name="renderedGroups">
			<xsl:for-each select="$renderingCommandsForElement[z:instruction/*/@z-mode='bottom']">
			...
			<xsl:for-each select="$renderingCommandsForElement[z:instruction/*/@z-mode='normal' or not(z:instruction/*/@z-mode)]">
			...
			<xsl:for-each select="$renderingCommandsForElement[z:instruction/*/@z-mode='top']">
				<xsl:sort select="@z-index" data-type="number"/>
				<xsl:call-template name="renderCommand">
					<xsl:with-param name="command" select="."/>
				</xsl:call-template>
		...
		<xsl:if test="normalize-space($renderedGroups)">
			<g class="groupedByRule" osm:id="{$osmId}">
				<xsl:copy-of select="$renderedGroups/."/>
			</g>
		</xsl:if>
	</xsl:for-each>-->
  ...
  <xsl:template name='rule'>
    ...
    <xsl:value-of select="count($selectedElements)"/>
    </xsl:message>
  ...
  <xsl:template name="processElements">
  	<!-- Attribute values from style file -->
    <xsl:param name="eBare"/>
    <xsl:param name="kBare"/>
    <xsl:param name="vBare"/>
    <!-- Elements that current rules applies to -->
    <xsl:param name="elements"/>
    <!-- Rule node -->
    <xsl:param name="rule"/>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:message>
			...
			<!-- Important: This calls individual painting commands -->
